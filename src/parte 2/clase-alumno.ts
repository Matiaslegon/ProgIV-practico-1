...
export class Alumno {
  ...
  constructor(legajo: number, nombre: string, apellido: string, edad: number, email: string) {
    this.legajo = legajo;
    this.nombre = nombre;
  ...
  setEdad(edad: number): void {
    if (edad < 0 || edad > 120) throw new Error("Debe ingresar una edad valida entre 0 y 120");
    this.edad = edad;
  }
  ...
  quitarMateria(codigo: number): Materia | undefined {
    const materiaAEliminar = this.materias.find((materia) => materia.codigo === codigo);
    this.materias = this.materias.filter((materia) => materia !== materiaAEliminar);

    return materiaAEliminar;
  }
